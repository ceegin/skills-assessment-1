...
def smallest_int(numbers):
    ...
    **DO NOT USE** the built-in function `min()`!
    ...
    If the input list is empty, return `None`::
        ...
        True
    """
    #why didn't this work? numbers.sort() return numbers[0]

    smallest_int_list = sorted(numbers)
    if smallest_int_list:
...
def join_strings_with_comma(words):
    ...
    return (", ").join(words) #not sure how to do this without .join
...
def duplicates(items):
    ...
    for fruit in items:
        ...
        elif fruit not in duplicate_list:
            duplicate_list.append(fruit)
